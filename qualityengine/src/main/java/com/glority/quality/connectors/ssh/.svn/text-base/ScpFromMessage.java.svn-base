...
public class ScpFromMessage extends AbstractSshMessage {
     ...
     */
    public ScpFromMessage(boolean verbose, Session session, String aRemoteFile,
            File aLocalFile, boolean recursive, boolean preserveLastModified) {
        super(verbose, session);
    ...
    private void startRemoteCpProtocol(InputStream in, OutputStream out,
            File localFile) throws IOException, JSchException {
    ...
    private void parseAndFetchFile(String serverResponse, File localFile,
            OutputStream out, InputStream in) throws IOException, JSchException {
        int start = 0;
    ...
    private void fetchFile(File localFile, long filesize, OutputStream out,
        ...
        try {
            while (true) {
                length = in.read(buf, 0, (BUFFER_SIZE < tempFilesize) ? BUFFER_SIZE
                        : (int) filesize);
                if (length < 0) {
    ...
    private void setLastModified(File localFile) throws JSchException {
        ...
        FileUtils.getFileUtils().setFileLastModified(localFile,
                ((long) fileAttributes.getMTime()) * ONE_SECOND);
    }
