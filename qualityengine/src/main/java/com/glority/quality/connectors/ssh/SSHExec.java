...
public class SSHExec extends SSHBase {
    ...
    private File commandResource = null;

    private OutputStream outStream;
    private int exitCode = SSH_EXIT_CODE_NOT_SETTED;
    ...
    public void setCommandResource(String f) {
        this.commandResource = new File(f);
    }
     ...
     * 
     * @param inputProperty
     *            The property which contains the input data for the remote
     *            command.
     * 
     * @since Ant 1.8.0
     */
     ...
     * 
     * @param inputString
     *            the input data for the remote command.
     * 
     * @since Ant 1.8.3
     */
     ...
     * <code>setOutput</code>. Default is false, that is, overwrite the file.
     * 
     * @param append
     ...
     * 
     * @param property
     *            The name of the property in which the command output will be
     *            stored.
     */
    ...
    public void process() {
        ...
        if (StringUtil.isEmpty(command) && commandResource == null) {
            throw new BuildException("Command or commandResource is required.");
        }

        int numberOfInputs = (inputFile != null ? 1 : 0) + (inputProperty != null ? 1 : 0)
                + (inputString != null ? 1 : 0);
        if (numberOfInputs > 1) {
            throw new BuildException("You can't specify more than one of" + " inputFile, inputProperty and"
                    + " inputString.");
        }
        ...
        try {
            ...
            if (!StringUtil.isEmpty(command)) {
                ...
                try {
                    BufferedReader br = new BufferedReader(new FileReader(commandResource));
                    String cmd;
    ...
    private void executeCommand(Session session, String cmd, StringBuffer sb) {
        TeeOutputStream tee = new TeeOutputStream(outStream, new KeepAliveOutputStream(System.out));
        ...
        try {
                ...
                public void run() {
                    while (!channel.isClosed()) {
                        ...
                        try {
                            ...
                            getProject().log("Sleep interuptted. this can be safely ignored");
                        }
                    }
                }
            };
            ...
            if (thread.isAlive()) {
                ...
                thread = null;
                if (failOnError) {
                    this.exitCode = SSH_EXIT_CODE_FAIL;
                    throw new BuildException(TIMEOUT_MESSAGE);
                } else {
                    this.exitCode = SSH_EXIT_CODE_TIMEOUT;
                    log(TIMEOUT_MESSAGE, Project.MSG_ERR);
                }
            } else {
                // success
                this.exitCode = SSH_EXIT_CODE_SUCCESS;
                if (outputFile != null) {
                    writeToFile(outStream.toString(), append, outputFile);
                }
