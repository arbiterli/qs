...
public abstract class AbstractSshMessage {
    private static final Logger LOGGER = Logger.getLogger(AbstractSshMessage.class);
    ...
    protected void logStats(long timeStarted, long timeEnded, long totalLength) {
        ...
        listener.log("File transfer time: " + format.format(duration) + " Average Rate: "
     ...
     * 
     * @param filesize
     *            the size of the file been transmitted
     * @param totalLength
     *            the total transmission size
     * @param percentTransmitted
    ...
    protected final int trackProgress(long filesize, long totalLength, int percentTransmitted) {
        ...
        int percent = (int) Math.round(Math.floor((totalLength / (double) filesize) * PERCENT_100));

        if (percent > percentTransmitted) {
            if (filesize < MAX_LOG_FILE_SIZE) {
                if (percent % PERCENT_10 == 0) {
                ...
                if (percent == PERCENT_50) {
                    LOGGER.debug(" 50%");
                } else if (percent == PERCENT_100) {
                    LOGGER.debug(" 100%");
                } else {
                    LOGGER.debug(".");
                }
