...
/**
 * Establishes an ssh session with a remote machine, optionally establishing
 * port forwarding, then executes any nested task(s) before closing the session.
 * 
 * @since Ant 1.8.0
 */
public class SSHSession extends SSHBase {
    ...
    private static final int SPEC_INDEX_LIMIT = 3;

    private Vector<LocalTunnel> localTunnels = new Vector<LocalTunnel>();
    private Set<Integer> localPortsUsed = new TreeSet<Integer>();
    private Vector<RemoteTunnel> remoteTunnels = new Vector<RemoteTunnel>();
    private Set<Integer> remotePortsUsed = new TreeSet<Integer>();
    ...
    /**
     * Changes the comma-delimited list of local tunnels to establish on the
     * connection.
     * 
     * @param tunnels
     *            a comma-delimited list of lport:rhost:rport tunnel
     *            specifications
     */
    public void setLocaltunnels(String tunnels) {
        ...
        for (int i = 0; i < specs.length; i++) {
            if (specs[i].length() > 0) {
                String[] spec = specs[i].split(":", SPEC_INDEX_LIMIT);
    ...
    /**
     * Changes the comma-delimited list of remote tunnels to establish on the
     * connection.
     * 
     * @param tunnels
     *            a comma-delimited list of rport:lhost:lport tunnel
     *            specifications
     */
    ...
    /**
     * Local Tunnel objects which represents a local tunnel.
     * 
     * @author XQS
     * 
     */
    public class LocalTunnel {
        ...
        public void setLPort(int lport) {
            Integer portKey = Integer.valueOf(lport);
    ...
    /**
     * Remote Tunnel object which represents a remote tunnel.
     * 
     * @author XQS
     * 
     */
    public class RemoteTunnel {
        ...
        public void setRPort(int rport) {
            Integer portKey = Integer.valueOf(rport);
    ...
    /**
     * The class corresponding to the sequential nested element. This is a
     ...
     *                if one of the nested tasks fails, or network error or bad
     *                parameter.
     */
    @Override
    public void process() {
        ...
        try {
            ...
            for (Iterator<LocalTunnel> i = localTunnels.iterator(); i.hasNext();) {
                LocalTunnel tunnel = (LocalTunnel) i.next();
                session.setPortForwardingL(tunnel.getLPort(), tunnel.getRHost(), tunnel.getRPort());
            }

            for (Iterator<RemoteTunnel> i = remoteTunnels.iterator(); i.hasNext();) {
                RemoteTunnel tunnel = (RemoteTunnel) i.next();
                session.setPortForwardingR(tunnel.getRPort(), tunnel.getLHost(), tunnel.getLPort());
            }

            for (Iterator<Task> i = nestedSequential.getNested().iterator(); i.hasNext();) {
