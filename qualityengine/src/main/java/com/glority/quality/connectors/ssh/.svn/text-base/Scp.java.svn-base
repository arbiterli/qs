...
public class Scp extends SSHBase {
    ...
    private static final String[] FROM_ATTRS = { "file", "localfile",
            "remotefile" };

    private static final String[] TO_ATTRS = { "todir", "localtodir",
            "remotetodir", "localtofile", "remotetofile" };
    ...
    /**
     * Sets the file to be transferred. This can either be a remote file or a
     * local file. Remote files take the form:<br>
     * <i>user:password@host:/directory/path/file.example</i><br>
     * Files to transfer can also include a wildcard to include all files in a
     * remote directory. For example:<br>
    ...
    /**
     * Sets the location where files will be transferred to. This can either be
     * a remote directory or a local directory. Remote directories take the form
     * of:<br>
     * <i>user:password@host:/directory/path/</i><br>
    ...
    /**
     * Similiar to {@link #setFile setFile} but explicitly states that the file
     * is a local file. This is the only way to specify a local file with a @
     * character.
     * 
     * @param aFromUri
    ...
    /**
     * Similiar to {@link #setTodir setTodir} but explicitly states that the
     * directory is a local. This is the only way to specify a local directory
     * with a @ character.
     * 
     * @param aToUri
     *            a string representing the target of the copy.
    ...
    /**
     * Sets flag to determine if file timestamp from remote system is to be
    ...
    /**
     * Changes the file name to the given name while receiving it, only useful
     * if receiving a single file.
     * 
     * @param aToUri
     *            a string representing the target of the copy.
     * @since Ant 1.6.2
    ...
    /**
     * Adds a FileSet tranfer to remote host. NOTE: Either addFileSet() or
     * setFile() are required. But, not both.
     * 
     * @param set
        ...
        try {
            ...
            if (!isSftp) {
                message = new ScpFromMessage(getVerbose(), session, file,
                        ...
                        fromSshUri.endsWith("*"), preserveLastModified);
            } else {
    ...
    private void upload(List<FileSet> fileSet, String toSshUri)
            throws IOException, JSchException {
        String file = parseUri(toSshUri);

        Session session = null;
        try {
            List<Directory> list = new ArrayList<Directory>(fileSet.size());
            for (Iterator<FileSet> i = fileSet.iterator(); i.hasNext();) {
            ...
            if (!list.isEmpty()) {
                ...
                if (!isSftp) {
                    message = new ScpToMessage(getVerbose(), session, list,
                            file);
                } else {
    ...
    private void upload(String fromPath, String toSshUri) throws IOException,
            JSchException {
        String file = parseUri(toSshUri);
        ...
        try {
            ...
            ScpToMessage message = null;
            if (!isSftp) {
                message = new ScpToMessage(getVerbose(), session, getProject()
                        .resolveFile(fromPath), file);
            } else {
                message = new ScpToMessageBySftp(getVerbose(), session,
