...
@members
{
	...
	public	 CppTreeTreeParser(TreeNodeStream input,String filename) 
	{
   		this(input);
...
procLine [boolean condition]		
		...
		) ? e=End { if (condition == true) System.out.println("	// line	" + e.getLine() + " " + fileName); }
		;

fileInclusion	[ boolean condition]
@init { String filename=null;	}
	 	:	
		(	s=INCLUDE 		{	filename = s.toString();	}
		|	inc=INCLUDE_EXPAND	
			{
				...
				try
           		{
					...
					CppTreeTreeParser iwalker = new CppTreeTreeParser(inodes,filename);
					iwalker.preprocess();
           		}
           		catch (Exception ex)
           		{
