...
public class CPDConfiguration extends AbstractConfiguration {
    ...
    public CPDConfiguration(String[] args) {
        ...
        final String systemDefaultEncoding = System
                .getProperty("file.encoding");
        setEncoding(findOptionalStringValue(args, "--encoding",
                systemDefaultEncoding));

        minimumTileSize = Integer.parseInt(findRequiredStringValue(args,
                "--minimum-tokens"));

        skipDuplicates = findBooleanSwitch(args, "--skip-duplicate-files");
    }

    public CPDConfiguration(int theMinTileSize, Language theLanguage,
            String theEncoding) {
    ...
    public static String findOptionalStringValue(String[] args, String name,
            String defaultValue) {
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals(name)) {
                return args[i + 1];
