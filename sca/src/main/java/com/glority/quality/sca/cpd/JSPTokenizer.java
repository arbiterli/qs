...
public class JSPTokenizer implements Tokenizer {
    ...
    public void tokenize(SourceCode sourceCode, Tokens tokenEntries) {
        ...
        LanguageVersionHandler languageVersionHandler = Language.JSP
                .getDefaultVersion().getLanguageVersionHandler();
        TokenManager tokenMgr = languageVersionHandler.getParser(
                languageVersionHandler.getDefaultParserOptions())
                .getTokenManager(sourceCode.getFileName(),
                        new StringReader(buffer.toString()));
        Token currentToken = (Token) tokenMgr.getNextToken();
        while (currentToken.image.length() > 0) {
            tokenEntries.add(new TokenEntry(String.valueOf(currentToken.kind),
                    sourceCode.getFileName(), currentToken.beginLine, tokenEntries.getType()));
