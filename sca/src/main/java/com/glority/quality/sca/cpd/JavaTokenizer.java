...
public class JavaTokenizer implements Tokenizer {
    ...
    public void setProperties(Properties properties) {
        ignoreLiterals = Boolean.parseBoolean(properties.getProperty(
                IGNORE_LITERALS, "false"));
        ignoreIdentifiers = Boolean.parseBoolean(properties.getProperty(
    ...
    public void tokenize(SourceCode sourceCode, Tokens tokenEntries) {
        ...
        /*
         * I'm doing a sort of State pattern thing here where this goes into
         * "discarding" mode when it hits an import or package keyword and goes
         * back into "accumulate mode" when it hits a semicolon. This could
         * probably be turned into some objects.
         */
        // Note that Java version is irrelevant for tokenizing
        LanguageVersionHandler languageVersionHandler = LanguageVersion.JAVA_14
                .getLanguageVersionHandler();
        String fileName = sourceCode.getFileName();
        TokenManager tokenMgr = languageVersionHandler.getParser(
                languageVersionHandler.getDefaultParserOptions())
                .getTokenManager(fileName, new StringReader(buffer.toString()));
        Token currentToken = (Token) tokenMgr.getNextToken();
        boolean inDiscardingState = false;
        while (currentToken.image.length() > 0) {
            if (currentToken.kind == JavaParserConstants.IMPORT
                    || currentToken.kind == JavaParserConstants.PACKAGE) {
            ...
            if (inDiscardingState
                    && currentToken.kind == JavaParserConstants.SEMICOLON) {
            ...
            if (currentToken.kind != JavaParserConstants.SEMICOLON) {
                ...
                tokenEntries.add(new TokenEntry(image, fileName,
                        currentToken.beginLine, tokenEntries.getType()));
            }
